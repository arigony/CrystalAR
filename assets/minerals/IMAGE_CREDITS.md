# Créditos das imagens minerais — CrystalAR 5.3.2

As imagens desta pasta são fotografias ou micrografias reais obtidas de fontes com licença de reutilização. Elas foram recortadas e redimensionadas para os cartões educacionais da CrystalAR. Os arquivos originais, autores, licenças e hashes estão registrados em `manifest.json`.

## Diamante — `diamond.jpg`

- Fotografia: **Rob Lavinsky, iRocks.com**
- Exemplar: diamante natural maclado (`macle`), triangular e transparente, `9,94 ct`, `1,5 × 1,5 × 0,4 cm`, África do Sul
- Fonte: <https://commons.wikimedia.org/wiki/File:Diamond-diamond_macle1.jpg>
- Licença: **CC BY-SA 3.0**
- Alterações: recorte quadrado, redimensionamento e compressão JPEG
- Nota didática: a forma triangular decorre da macla cristalina; o visualizador 3D apresenta a rede covalente periódica

## Grafite — `graphite.jpg`

- Fotografia: **Rob Lavinsky, iRocks.com**
- Exemplar: massa natural cinza-aço com forma lamelar e aspecto foliado
- Fonte: <https://commons.wikimedia.org/wiki/File:Graphite-233436.jpg>
- Licença: **CC BY-SA 3.0**
- Alterações: recorte quadrado, redimensionamento e compressão JPEG
- Nota didática: o aspecto foliado é usado para introduzir camadas, clivagem e deslizamento, sem afirmar que o hábito externo é uma imagem direta das folhas atômicas

## Rutilo — `rutile.jpg`

- Fotografia: **Rob Lavinsky, iRocks.com**
- Exemplar: cristais reticulados e estriados de rutilo, Diamantina, Minas Gerais, Brasil
- Fonte: <https://commons.wikimedia.org/wiki/File:Rutile-ww7c.jpg>
- Licença: **CC BY-SA 3.0**
- Alterações: recorte quadrado, redimensionamento e compressão JPEG

## Anatásio — `anatase.jpg`

- Fotografia: **Tom Epaminondas e Eurico Zimbres**
- Exemplar: cristais castanhos de anatásio, Gouveia, Minas Gerais, Brasil
- Fonte: <https://commons.wikimedia.org/wiki/File:Anat%C3%A1sio.jpeg>
- Licença: **CC BY-SA 2.0 BR**
- Alterações: recorte quadrado, redimensionamento e compressão JPEG

## Brookita — `brookite.jpg`

- Fotografia: **Assianir**
- Exemplar: brookita com hábito tabular/lamelar visível
- Fonte: <https://commons.wikimedia.org/wiki/File:Brookite.jpg>
- Licença: **CC BY-SA 3.0**
- Alterações: recorte quadrado, redimensionamento e compressão JPEG

## Calcita — `calcite.jpg`

- Fotografia: **Parent Géry**
- Exemplar: romboedros de calcita
- Fonte: <https://commons.wikimedia.org/wiki/File:Calcite_5.jpg>
- Licença: **Domínio público**
- Alterações: recorte quadrado, redimensionamento e compressão JPEG

## Aragonita — `aragonite.jpg`

- Fotografia: **Rob Lavinsky, iRocks.com**
- Exemplar: agregados radiais tridimensionais de aragonita, Nevada, EUA
- Fonte: <https://commons.wikimedia.org/wiki/File:Aragonite-23204.jpg>
- Licença: **CC BY-SA 3.0**
- Alterações: recorte quadrado, redimensionamento e compressão JPEG

## Vaterita — `vaterite.jpg`

- Micrografia eletrônica de varredura: **A. Di Falco**
- Conteúdo: esfera de vaterita em escala microscópica
- Fonte: <https://commons.wikimedia.org/wiki/File:Vaterite_Spheres_SEM.jpg>
- Licença: **CC BY 3.0**
- Alterações: seleção do painel `c`, recorte quadrado, redimensionamento e compressão JPEG
- Nota didática: esta imagem é uma **micrografia real**, e não uma fotografia de amostra macroscópica

## Política de uso

- A fotografia é apresentada como uma ocorrência ou morfologia representativa, não como consequência exclusiva da estrutura cristalina.
- Cor e hábito podem variar com impurezas, condições de crescimento, geminação e associação com outros minerais.
- A imagem macroscópica não representa uma visualização direta dos átomos.
- As licenças `CC BY-SA` exigem atribuição e compartilhamento de adaptações sob licença compatível.
