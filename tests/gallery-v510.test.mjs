...
test("package and scientific release are version 5.3.2", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  assert.equal(pkg.version, "5.3.2");
  const loader = await readFile(new URL("../src/runtime-watchdog.js", import.meta.url), "utf8");
  assert.match(loader, /science-v510\.js\?v=5\.3\.2/);
  assert.match(scienceModule, /const VERSION = "v5\.3\.2"/);
});

test("HTML-base exposes 5.3.2 before JavaScript enhancement", () => {
  assert.match(index, /Versão v5\.3\.2/);
  assert.match(index, /roteiro diamante-grafite/i);
  assert.match(index, /style-v510\.css\?v=5\.3\.2/);
  assert.match(index, /runtime-watchdog\.js\?v=5\.3\.2/);
  assert.match(index, /provenance\.js\?v=5\.3\.2/);
  assert.match(index, /app\.js\?v=5\.3\.2/);
