...
assert.match(dom, /Versão v5\.3\.2/, `A versão 5.3.2 não apareceu no DOM renderizado.\n${stderr}`);
...
assert.match(dom, /id="mineralImageButton"[^>]*aria-expanded="false"/, `O botão acessível da imagem mineral não apareceu.\n${stderr}`);
assert.match(dom, /id="mineralImage"/, `A imagem mineral não apareceu no roteiro.\n${stderr}`);
assert.match(dom, /assets\/minerals\/diamond\.jpg\?v=5\.3\.2/, `A fotografia de diamante não foi sincronizada ao roteiro inicial.\n${stderr}`);
assert.match(dom, /Diamante natural transparente com macla triangular do tipo macle/i, `O texto alternativo da fotografia de diamante está ausente.\n${stderr}`);
assert.match(dom, /Rob Lavinsky, iRocks.com/, `O crédito da imagem do diamante não apareceu.\n${stderr}`);
...
console.log("Browser smoke PASS: CrystalAR 5.3.2 e roteiro diamante-grafite carregados no navegador real.");
