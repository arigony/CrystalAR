...
const CACHE_NAME = "crystalar-cif-v3";
...
export function buildDatasetFilterURL(rawId) {
  const id = String(rawId || "").trim();
  if (!/^\d{7,8}$/.test(id)) throw new Error("COD ID inválido para consulta.");
  const url = new URL("https://datasets-server.huggingface.co/filter");
  url.searchParams.set("dataset", DATASET);
  url.searchParams.set("config", "default");
  url.searchParams.set("split", "train");
  url.searchParams.set("where", `\"file\"=${Number(id)}`);
...
async function fetchWithTimeout(url, options = {}, timeoutMs = 12000) {
  ...
  try {
    ...
    if (error?.name === "AbortError") throw new Error(`tempo limite de ${Math.round(timeoutMs / 1000)} s excedido`);
    throw error;
  } finally {
    clearTimeout(timer);
  }
...
async function fetchFromHuggingFace(id) {
  // COD IDs are integers. An exact /filter predicate is substantially faster and
  // more reliable than a BM25 full-text /search across 533k large CIF strings.
  const url = buildDatasetFilterURL(id);
  const response = await fetchWithTimeout(url, { mode: "cors" }, 12000);
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const payload = await response.json();
  if (payload?.partial === true) throw new Error("índice parcial do espelho");
  const cif = extractCIFTextFromDatasetResponse(payload, id);
  if (!cif) throw new Error("COD ID exato não encontrado no espelho");
...
async function fetchFromOfficialCOD(id) {
  ...
  const tasks = urls.map(async url => {
    const response = await fetchWithTimeout(url, { mode: "cors" }, 7000);
  ...
  try {
    ...
    const messages = (error?.errors || []).map(item => item?.message || "falha de rede/CORS");
    throw new Error(messages.join("; ") || "servidores oficiais indisponíveis no navegador");
  }
}
...
async function fetchRemoteFirst(id) {
  const attempts = [
    [`espelho COD no Hugging Face (${DATASET_SNAPSHOT})`, fetchFromHuggingFace],
    ["servidores oficiais COD", fetchFromOfficialCOD]
  ];
  if (configuredProxyUrl(id)) attempts.unshift(["proxy CrystalAR", fetchFromConfiguredProxy]);

  const jobs = attempts.map(async ([source, loader]) => ({
    text: await loader(id),
    source
  }));
  ...
  try {
    ...
    const details = (error?.errors || []).map((item, index) => `${attempts[index]?.[0] || "fonte"}: ${item?.message || "falhou"}`);
    throw new Error(details.join(" | ") || "todas as fontes remotas falharam");
  }
}
...
export async function fetchCIFById(rawId) {
  ...
  const result = await fetchRemoteFirst(id);
  await putCached(id, result.text);
  return result;
}
