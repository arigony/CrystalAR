...
export const ELEMENT_COLORS = {
  ...
  Na: 0x7d55d9, K: 0x8b3fe3, Mg: 0x3cc35c, Ca: 0x45cc45, Fe: 0xd56a36, Co: 0xef5b8c,
...
export function makeCellVectors(cell) {
  ...
  const cxVector = [cx, cy, Math.sqrt(czSquared)];
  return [ax, bx, cxVector];
}
...
export function buildCrystalModel(doc, repeat = 1) {
  ...
  const cell = extractCell(doc);
  const vectors = makeCellVectors(cell);
  const asymmetricAtoms = extractAtomSites(doc);
  const symmetryOperations = extractSymmetryOperations(doc);
  const unitAtoms = expandAsymmetricUnit(asymmetricAtoms, symmetryOperations);
  const atoms = [];
...
export function inferBonds(atoms, maxBonds = 18000) {
  ...
  for (let index = 0; index < atoms.length; index += 1) {
    ...
    const base = atom.cart.map(v => Math.floor(v / cellSize));
    for (const dx of offsets) for (const dy of offsets) for (const dz of offsets) {
      ...
      for (const otherIndex of candidates) {
        ...
        const x = atom.cart[0] - other.cart[0];
        const y = atom.cart[1] - other.cart[1];
        const z = atom.cart[2] - other.cart[2];
        const distance = Math.hypot(x, y, z);
        const cutoff = ((COVALENT_RADII[atom.element] || 0.9) + (COVALENT_RADII[other.element] || 0.9)) * 1.24;
