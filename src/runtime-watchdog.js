(() => {
  ...
  const scienceModuleUrl = new URL("./science-v510.js?v=5.3.1", document.currentScript.src).href;
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    import(scienceModuleUrl).catch(error => { console.error("CrystalAR 5.3.1 scientific module:", error); const status=$("status"); if(status) status.textContent=`Galeria base ativa; módulo científico 5.3.1 não iniciou: ${error.message}`; });
